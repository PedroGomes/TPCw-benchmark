...
public class CassandraPopulator extends AbstractBenchmarkPopulator {
	...
	public boolean populate() {
		...
		if (error) {
			...
			try {
				...
				long cool_down = 60000;
				System.out.println("Cooling down for " + (cool_down / 1000) + "s");
				Thread.sleep(cool_down);


			} catch (InterruptedException ex) {
				Logger.getLogger(CassandraPopulator.class.getName()).log(Level.SEVERE, null, ex);
				return false;
			} catch (Exception ex) {
				Logger.getLogger(CassandraPopulator.class.getName()).log(Level.SEVERE, null, ex);
				return false;
...
//            insert(country.getCo_id(), key, "Addresses", "ADDR_CO_ID", writeConsistency);
	...
	public void insertOrder_and_CC_XACTS(int n) throws InterruptedException {
		...
		for (int i = threads; i > 0; i--) {
			...
			int base = (threads - i);	  ///code copy form above constructors, if reactivated please revise
	...
	class Order_and_XACTSPopulator implements Runnable {
		...
		public void databaseInsert(Order order, List<OrderLine> orderLines, org.uminho.gsd.benchmarks.TPCW_Cassandra.entities.CCXact ccXact) throws Exception {
			if (client instanceof TPCWCassandraExecutor) {
...
//
			...
			for (int z = 0; z < number_keys; z++) {
				...
				Order order = new Order(order_id, O_C_ID, O_DATE, O_SUB_TOTAL, O_TAX, O_TOTAL, O_SHIP_TYPE, O_SHIP_DATE, O_STATUS, billAddress, O_SHIP_ADDR);
