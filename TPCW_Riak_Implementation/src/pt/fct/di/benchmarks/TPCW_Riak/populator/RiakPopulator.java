 ...
 * ********************************************************************
 */
package pt.fct.di.benchmarks.TPCW_Riak.populator;
...
import pt.fct.di.benchmarks.TPCW_Riak.entities.Country;
...
public class RiakPopulator extends AbstractBenchmarkPopulator {
	...
	public RiakPopulator(
		...
		if (do_delays == null || do_delays.isEmpty()) {
			delay_inserts = false;
			System.out
					.println("[WARN:] NO DELAY OPTION DEFINED, DELAYS NOT USED");
		} else {
		...
		if (delay_inserts) {
			...
			if (delay_time_info == null || delay_time_info.isEmpty()) {
				delay_time = 10;
				System.out
						.println("[WARN:] NO DELAY TIME DEFINED, DEFAULT: 10ms");
			} else {
...
//		Constants.NUM_AUTHORS = 50;
//		Constants.NUM_ITEMS = 200;
//		Constants.NUM_ORDERS = 500;
	...
	public boolean populate() {
		...
		if (error) {
			...
			try {
				...
				long cool_down = 6000;
				System.out.println("Cooling down for " + (cool_down / 1000)
						+ "s");
				Thread.sleep(cool_down);

			} catch (InterruptedException ex) {
				Logger.getLogger(RiakPopulator.class.getName()).log(
	...
	public void removeALL() throws Exception {
		...
		client.truncate("item_subject_index");
		client.truncate("item_title_index");
		client.truncate("item_author_index");
		client.truncate("Item");
		client.truncate("Orders");
		client.truncate("Order_Line");
		client.truncate("CC_XACTS");
	...
	public void databaseInsert(DatabaseExecutorInterface client,
			String Operation, String key, String path, Entity value,
			ResultHandler results) throws Exception {
	...
	class AuthorPopulator implements Runnable {
		...
		public void databaseInsert(String Operation, String key, String path,
		...
		public void insertAuthors(int n) {
			...
			for (int i = 0; i < n; i++) {
				...
				// insert(first_name, key, "Author", "A_FNAME",
				// writeConsistency);
				// insert(last_name, key, "Author", "A_LNAME",
				// writeConsistency);
				// insert(middle_name, key, "Author", "A_MNAME",
				// writeConsistency);
				...
				Author a = new Author(base + i, first_name, last_name,
						middle_name, dob.toString(), bio);
				if (insertDB)
					try {
						databaseInsert("INSERT_Authors", (base + i) + "",
								"author", a, partial_results);
					} catch (Exception e) {
						e.printStackTrace();
						client_error = true;
						break;
	...
	class CustomerPopulator implements Runnable {
		...
		public void insertCustomers(int n) {
			...
			for (int i = 0; i < n; i++) {
				...
				String pass = names[0].charAt(0) + names[1].charAt(0) + ""
						+ random_int;
				...
				java.sql.Date C_SINCE = new java.sql.Date(cal.getTime()
						.getTime());
				...
				java.sql.Date C_LAST_LOGIN = new java.sql.Date(cal.getTime()
						.getTime());
				...
				java.sql.Timestamp C_EXPIRATION = new java.sql.Timestamp(cal
						.getTime().getTime());
				// insert(C_EXPIRATION, key, "Customer", "C_EXPIRATION",
				// writeCon);

				cal = BenchmarkUtil.getRandomDate(1880, 2000);
				java.sql.Date C_BIRTHDATE = new java.sql.Date(cal.getTime()
						.getTime());
				...
				String address_id = addresses
						.get(rand.nextInt(addresses.size()));
				// insert(address.getAddr_id(), key, "Customer", "C_ADDR_ID",
				// writeCon);

				Customer c = new Customer(base + i + "", key, pass, last_name,
						first_name, phone + "", email, C_SINCE.toString(), C_LAST_LOGIN.toString(),
						C_LOGIN.toString(), C_EXPIRATION.toString(), C_BALANCE, C_YTD_PMT,
						C_BIRTHDATE.toString(), C_DATA, discount, address_id);

				try {
					databaseInsert("INSERT_Customers", (base + i) + "",
							"customer", c, partial_results);
				} catch (Exception e) {
					e.printStackTrace();
					client_error = true;
					break;
	...
	class ItemPopulator implements Runnable {
		...
		public void insertItems(int n) {
			...
			for (int i = 0; i < n; i++) {
				...
				java.sql.Date pubDate = new java.sql.Date(
						System.currentTimeMillis());

				String thumbnail = new String("img" + i % 100 + "/thumb_" + i
						+ ".gif");
				String image = new String("img" + i % 100 + "/image_" + i
						+ ".gif");
				...
				java.sql.Date avail = new java.sql.Date(
						System.currentTimeMillis() + rand.nextInt(1200000)); // Data
																				// when
																				// available

				String dimensions = ((double) BenchmarkUtil.getRandomInt(1,
						9999) / 100.0)
						+ "x"
				...
				Item item = new Item(base + i, I_TITLE, pubDate.toString(), I_PUBLISHER,
						I_DESC, I_SUBJECT, thumbnail, image, I_COST, I_STOCK,
						isbn, srp, I_RELATED[0], I_RELATED[1], I_RELATED[2],
						I_RELATED[3], I_RELATED[4], I_PAGE, avail.toString(), I_BACKING,
						dimensions, author);

				try {
					databaseInsert("INSERT_Items", (base + i) + "",
							column_family, item, partial_results);
				} catch (Exception e) {
					e.printStackTrace();
					client_error = true;
					break;
				...
				if (build_indexes) {
					...
					try {
						client.index(I_SUBJECT, "item_subject_index",
								(time_stamp) + "." + (base + i), index_values);
						client.index(I_TITLE, "item_title_index", (base + i)
								+ "", index_values);
						client.index(I_AUTHOR.getA_LNAME(),
								"item_author_index", (base + i) + "",
								index_values);
					} catch (Exception e) {
						e.printStackTrace();
						client_error = true;
	...
	class AddressPopulator implements Runnable {
		...
		private void insertAddress(int n) {
			...
			for (int i = 0; i < n; i++) {
				...
				String key = ADDR_STREET1 + ADDR_STREET2 + ADDR_CITY
				...
				if (insertDB) {
					try {
						databaseInsert("INSERT_Addresses", key + "", "address",
								address, partial_results);
					} catch (Exception e) {
						e.printStackTrace();
						client_error = true;
						break;
	...
	private void insertCountries(int numCountries) {
		...
		for (int i = 0; i < numCountries; i++) {
			...
			Country country = new Country(i, countriesNames[i], currencies[i],
					exchanges[i]);
			try {
				databaseInsert(client, "INSERT_Countries", i + "", "country",
						country, results);
			} catch (Exception e) {
				e.printStackTrace();
				client_error = true;
	...
	class Order_and_XACTSPopulator implements Runnable {
		...
		public void databaseInsert(Order order, List<OrderLine> orderLines,
				pt.fct.di.benchmarks.TPCW_Riak.entities.CCXactItem ccXact)
				throws Exception {
			((TPCW_Riak_Executor) client).insertOrder(order, orderLines, ccXact);
		...
		public void insertOrder_and_CC_XACTS(int number_keys) {
			...
			for (int z = 0; z < number_keys; z++) {
				...
				String Customer_id = customers.get(rand.nextInt(customers
						.size()));
				...
				String billAddress = addresses.get(BenchmarkUtil.getRandomInt(
						0, NUM_ADDRESSES - 1));
				// insertInSuperColumn(billAddress.getAddr_id(), O_C_ID,
				// column_family, O_ID + "", "O_BILL_ADDR_ID", write_con);

				O_SHIP_ADDR = addresses.get(BenchmarkUtil.getRandomInt(0,
						NUM_ADDRESSES - 1));
				...
				Order order = new Order(order_id, O_C_ID, O_DATE.toString(), O_SUB_TOTAL,
						O_TAX, O_TOTAL, O_SHIP_TYPE, O_SHIP_DATE.toString(), O_STATUS,
						billAddress, O_SHIP_ADDR);
				...
				for (int i = 0; i < number_of_items; i++) {
					...
					OrderLine orderline = new OrderLine(OL_ID, order_id,
				...
				int country_id = countries.get(BenchmarkUtil.getRandomInt(0,
						countries.size() - 1));
				// insert(country.getCo_id(), key, column_family, "CX_CO_ID",
				// write_con);

				CCXactItem ccXact = new CCXactItem(CX_TYPE, CX_NUM, CX_NAME, CX_EXPIRY.toString(),	O_TOTAL, O_SHIP_DATE.toString(),order_id,
						country_id);
